...
uniform LightParameters directional;
...
vec4 computeAmbient(){
	...
	vec3 Ca = vec3(directional.La * material1.Ka);
...
vec4 computeDiffuse(vec3 L, vec3 N){
	...
	vec3 Cd = vec3(directional.Ld * material1.Kd);
...
vec4 computeSpecular(vec3 N, vec3 R, vec3 V){
	...
	vec3 Cs = vec3(directional.Ls*material1.Ks);
...
{	
	...
	vec3 vector_L = normalize(directional.Position);
	vec3 vector_N = v_normal;
	vec3 vector_R = normalize(reflect(-vector_L,vector_N));
	vec3 vector_V = normalize(v_camerapos);
