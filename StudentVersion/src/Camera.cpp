...
{
	// EXERCISE PRACT 1
	Vector3 delta_local = getLocalVector(delta);

	center = center + delta_local;
	eye = eye + delta_local;

	updateViewMatrix();
}

//Function to move the camera by using WASD keys
void Camera::moveWASD(Vector3 direction) {

	//First we get the local vector of the direction where our camera is going to move
	Vector3 local_direction = getLocalVector(direction);

	//Then we just update center and eye
	center = center - local_direction;
	eye = eye - local_direction;
	updateViewMatrix();
}
...
{
	// EXERCISE PRACT 1
	Vector3 axis_local = getLocalVector(axis);

	Vector3 new_front;
	Matrix44 R;
	R.setRotation(angle, axis);
	new_front = R * (center - eye);
...
void Camera::getPosition() {
	std::cout << "Actual position (" << eye.x << "," << eye.y << "," << eye.z << ")" << std::endl;
}
